...
function TaskDashboard() {
  ...
  const handleEditTaskSubmit = async (e) => {
    ...
    try {
      const response = await authFetch(`/api/tasks/${editTaskForm.id}`, {
        method: 'PUT',
        body: JSON.stringify({
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
              ...
              <div style={{ overflow: 'hidden' }}>
                <div style={{ fontWeight: 800, fontSize: '14px', color: '#fff', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                  {user?.name || 'Player'}
                </div>
                <div style={{ fontSize: '11px', color: '#A0AEC0', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                  {user?.email}
                </div>
              </div>
            </div>
          ...
          <div className="level-box">
            ...
            <div className="motivate-text">Keep going, {user?.name?.split(' ')[0] || 'champ'}! 💪</div>
          </div>
        </div>
      </aside>
